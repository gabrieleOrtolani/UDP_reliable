...
int verbose =0;
int blocking = 1;
int reliable =0; // se 1 non usa reliable
        ...
        if(FD_ISSET(socket_fd, &rfds)){
            ...
            if (atoi(recv_packet.ack_n) == expected_ack_number){          
                ...
                if (atoi(recv_packet.ack_n) < expected_ack_number){ //loss detecteed
                    ...
                    if(verbose)(printf("Wrong ACK #%d ...\tExpecting 2 be %d, sending from #%d %dseqs...\n",atoi(recv_packet.ack_n), expected_ack_number,seq2send, cwnd));
                    ...
                    cwnd = lastcwnd ;
                    seq2send = atoi(recv_packet.ack_n);
                    if(verbose)(printf("GAP ACK #%d ...\tExpecting 2 be %d, sending from #%d %dseqs...\n",atoi(recv_packet.ack_n), expected_ack_number,seq2send, cwnd));
