...
int rcvfrom_r(int socket_fd, void *buffer, size_t length, struct sockaddr *client_addr_0, socklen_t* client_len_0){
    ...
    int buffMSS =((int)((float)length/(float)MSS))+1;
    ...
    *client_len = sizeof(struct sockaddr);
    ...
    while (1) {
        ...
        while(FD_ISSET(socket_fd, &rfds)){
            ...
            if (atoi(recv_packet.seq_n) != expected_seq_n && atoi(recv_packet.seq_n) < buffMSS){
                if(verbose){
                    printf("Unexpected seq #%d - ", atoi(recv_packet.seq_n));
                    fflush(stdout);
                }
                gap = 1;
            }else if (atoi(recv_packet.seq_n) == expected_seq_n && atoi(recv_packet.seq_n)< buffMSS){
                if(verbose){
            ...
            if(atoi(recv_packet.seq_n) < buffMSS ){
                ...
                if(verbose){printf("Not saving... %d > %d\n",atoi(recv_packet.seq_n), buffMSS);}
                count++;
                gap = 0;
            }
